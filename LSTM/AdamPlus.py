...
class AdamPlus(Optimizer):
    ...
    def __setstate__(self, state):
        super(AdamPlus, self).__setstate__(state)
    ...
    def reset(self):
        for group in self.param_groups:
            for p in group['params']:
                ...
                state['exp_ref_avg'] = torch.zeros_like(p.data,memory_format=torch.preserve_format) \
                    if version_higher else torch.zeros_like(p.data)
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                exp_avg, exp_ref_avg, exp_avg_var = state['exp_avg'], state['exp_ref_avg'], state['exp_avg_var']
                ...
                exp_ref_avg.mul_(beta2).add_(grad, alpha=1 - beta2)
                grad_residual = grad - exp_ref_avg
