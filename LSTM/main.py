...
args.save = args.save + '-niter-{}'.format(args.epochs) + '-optimizer-{}'.format(args.optimizer) + '-nlayers{}'.format(args.nlayers) + \
            ...
            +'-epsqrt{}'.format(args.eps_sqrt) + '-betas-{}-{}'.format(args.beta1, args.beta2) + '-run{}'.format(args.run) + \
                '-wdecay{}-when-{}'.format(args.wdecay, args.when) + '-seed{}'.format(args.seed)
...
try:
    ...
    if args.optimizer.lower() == 'adabelief':
        optimizer = AdaBelief(params, lr=args.lr, weight_decay=args.wdecay,
                             eps=args.eps, betas=(args.beta1, args.beta2), rectify=False, weight_decouple=False)
    elif args.optimizer == 'adam+':
        optimizer = AdamPlus(params, lr=args.lr, weight_decay=args.wdecay,
                             eps=args.eps, betas=(args.beta1, args.beta2), rectify=False, weight_decouple=False)
