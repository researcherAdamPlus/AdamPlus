...
nlayer=1
# lr=0.001
for lr in 0.003 0.004 0.005 ; do
  for seed in {141..145}; do
    ...
    CUDA_VISIBLE_DEVICES=0 python -u main.py --batch_size 20 --data data/penn --dropouti 0.4 --dropouth 0.25 \
      ...
      --beta2 0.999 --optimizer adabelief --lr "$lr" --eps 1e-16 --eps_sqrt 0.0 \
      --nlayer ${nlayer} --run "$run" 2>&1 | tee -a "logs/run_0_seed_${seed}_nlayer_${nlayer}_lr_${lr}_adabelief.log"
  done
done